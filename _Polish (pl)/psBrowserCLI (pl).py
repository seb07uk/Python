...
__group__ = "python"
__desc__ = "psBrowser CLI: Edycja Samodzielna"

# --- KONFIGURACJA ŚCIEŻEK ---
# Użycie standardowej struktury katalogów Polsoft
BASE_DIR = os.path.join(os.environ['USERPROFILE'], ".polsoft", "psCli", "Browser")
SCREENSHOT_DIR = os.path.join(BASE_DIR, "Zrzuty")
DOWNLOAD_DIR = os.path.join(BASE_DIR, "Pobrane")
COOKIE_FILE = os.path.join(BASE_DIR, "cookies.txt")
HISTORY_FILE = os.path.join(BASE_DIR, "historia.json")
...
def download_file(url):
    try:
        ...
        print(f"{Color.YELLOW}[POBIERANIE]{Color.RESET} Pobieram: {url}...")
        with urllib.request.urlopen(url) as response, open(local_filename, 'wb') as out_file:
            shutil.copyfileobj(response, out_file)
        print(f"{Color.GREEN}[OK]{Color.RESET} Zapisano w folderze Pobrane.")
    except Exception as e:
        print(f"{Color.RED}[BŁĄD]{Color.RESET} {e}")
...
def show_about():
    ...
    print(f"{Color.BOLD}{Color.BLUE}--- psBrowser CLI (Standalone) ---{Color.RESET}")
    print(f"{Color.WHITE}autor:  Sebastian Januchowski")
...
def browse(url=None):
    history_stack = []
    current_url = url or "google.pl"

    while current_url and current_url.lower() not in ['q', 'quit', 'wyjdz']:
        if current_url.lower() == 'about':
            show_about()
            current_url = history_stack[-1] if history_stack else "google.pl"
            continue
        ...
        if current_url.lower() == 'h':
            if os.path.exists(HISTORY_FILE):
                with open(HISTORY_FILE, "r", encoding="utf-8") as f: 
                    h_list = json.load(f)
                    print(f"\n{Color.BOLD}OSTATNIA HISTORIA:{Color.RESET}")
        ...
        if res[0] == "DOWNLOAD":
            current_url = history_stack.pop() if history_stack else "google.pl"
            continue
