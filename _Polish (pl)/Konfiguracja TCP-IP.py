...
<body>
    ...
    <div style="font-size: 0.9em; color: #888; margin-top: 5px;">Data raportu: {datetime.datetime.now().strftime('%d.%m.%Y %H:%M:%S')}</div>
    <pre>{content}</pre>
    <div class="footer">Raport wygenerowany automatycznie przez system TERMINAL CLI</div>
</body>
</html>
"""
...
def get_full_ipconfig():
    os.system('') # Aktywacja ANSI w Windows
    report_content = ""
    target_path = os.path.expandvars(r"%userprofile%\.polsoft\psCLI\reports\network_report.html")

    print(f"{Colors.BOLD}{Colors.HEADER}--- Pełna Konfiguracja TCP/IP (TERMINAL CLI) ---{Colors.ENDC}\n")

    try:
        # Próba cp852 (standard Windows PL), jeśli zawiedzie - utf-8
        ...
        for line in lines:
            ...
            elif any(k in line for k in ["IPv4", "Maska", "Brama", "Subnet", "Gateway"]):
        ...
        print(f"\n{Colors.CYAN}[OK] Raport zapisany pomyślnie w:{Colors.ENDC}")
        print(f"{Colors.YELLOW}{target_path}{Colors.ENDC}")

    except subprocess.CalledProcessError:
        print(f"{Colors.RED}[!] Błąd: Nie udało się pobrać danych z ipconfig.{Colors.ENDC}")
    except Exception as e:
        print(f"{Colors.RED}[!] Błąd krytyczny: {e}{Colors.ENDC}")

    finally:
        print("\n" + f"{Colors.BLUE}" + "-"*45 + f"{Colors.ENDC}")
        input("Naciśnij Enter, aby powrócić do TERMINAL CLI...")
