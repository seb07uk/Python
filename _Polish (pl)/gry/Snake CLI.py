...
# Włączenie obsługi kolorów ANSI dla TERMINAL CLI
...
difficulty = {"level": 1, "speed": 0.12, "multiplier": 1, "name": "Łatwy"}
...
def show_options():
    ...
    while True:
        os.system("cls")
        print(f"{BLUE}=== OPCJE I USTAWIENIA ==={RESET}\n")
        print(f"{WHITE}Aktualny Poziom: {YELLOW}{difficulty['name']}{RESET}")
        print(f"{WHITE}Mnożnik punktów: {YELLOW}x{difficulty['multiplier']}{RESET}\n")
        print(f"{WHITE}[1] Poziom 1 (Łatwy)    - 1 pkt")
        print(f"[2] Poziom 2 (Normalny) - 2 pkt")
        print(f"[3] Poziom 3 (Trudny)   - 3 pkt")
        print(f"\n{RED}[R] RESET RANKINGU{RESET}")
        print(f"{BLUE}[B] Wróć do menu{RESET}")
        c = msvcrt.getch().decode().upper()
        if c == '1': difficulty = {"level": 1, "speed": 0.12, "multiplier": 1, "name": "Łatwy"}
        elif c == '2': difficulty = {"level": 2, "speed": 0.08, "multiplier": 2, "name": "Normalny"}
        elif c == '3': difficulty = {"level": 3, "speed": 0.04, "multiplier": 3, "name": "Trudny"}
        elif c == 'R':
            print(f"\n{RED}Czy na pewno wyczyścić ranking? (T/N){RESET}")
            if msvcrt.getch().decode().upper() == 'T':
                if os.path.exists(SNAKE_DATA): os.remove(SNAKE_DATA)
                print(f"{GREEN}Wyczyszczono!{RESET}")
                time.sleep(1)
        elif c == 'B': break

def show_top5():
    os.system("cls")
    print(f"{BLUE}=== TOP 5 NAJLEPSZYCH ==={RESET}\n")
    scores = load_scores()[:5]
    if not scores: print(f"{WHITE}Brak zapisanych wyników.{RESET}")
    for i, s in enumerate(scores, 1):
        print(f"{WHITE}{i}. {s['name']:<12} - {s['score']:03} pkt{RESET}")
    print(f"\n{BLUE}Naciśnij dowolny klawisz...{RESET}")
    msvcrt.getch()
...
class SnakeGame:
    ...
    def render(self, s1, s2):
        ...
        for y in range(1, self.h + 1):
            ...
            buf += row + "\n"
        buf += f"{RESET}{WHITE}Wynik: {self.current_score} | Poz: {difficulty['level']} | Q: Wyjdź{RESET}"
...
def show_help():
    os.system("cls")
    print(f"{BLUE}=== SNAKE CLI: O GRZE ==={RESET}")
    print(f"{WHITE}Snake CLI to klasyczna gra zręcznościowa")
    print("przeniesiona do środowiska Python.")
    print("Wcielasz się w cyfrowego węża, którego")
    print("jedynym celem jest konsumpcja danych (■).")
    print("Im więcej jesz, tym stajesz się dłuższy")
    print(f"i tym trudniej jest uniknąć kolizji.{RESET}")
    print(f"\n{BLUE}=== INSTRUKCJA ==={RESET}")
    print(f"{WHITE}Ruch:       W, A, S, D")
    print("Wyjście:    Q (w trakcie gry)")
    print(f"Punktacja:  Poz 1=1pkt, 2=2pkt, 3=3pkt{RESET}")
    print(f"\n{BLUE}-------------------------------{RESET}")
    ...
    print(f"{BLUE}-------------------------------{RESET}")
    print(f"\n{WHITE}Dowolny klawisz, aby wrócić...{RESET}")
    msvcrt.getch()
...
def start_game_flow():
    os.system("cls")
    sys.stdout.write(SHOW)
    name = input(f"{WHITE}Podaj imię: {RESET}").strip()[:10] or "Gracz"
    sys.stdout.write(HIDE)
    
    while True:
        os.system("cls")
        draw_logo()
        print(f"\n{WHITE}Wybierz skórkę dla swojego węża:{RESET}")
    ...
    while True:
        ...
        print(f"\n{RED}KONIEC GRY!{RESET} Twój wynik: {game.current_score}")
        print(f"{WHITE}Czy chcesz zagrać ponownie? (T/N){RESET}")
        if msvcrt.getch().decode().upper() != 'T': return
