...
# Konfiguracja ścieżek zgodnie z Twoim TERMINAL CLI
...
class Tetris:
    def __init__(self, root):
        ...
        self.state = "MENU"
        self.menu_index = 0
        self.menu_options = ["NOWA GRA", "TOP 5", "POMOC", "WYJSCIE"]
    ...
    def check_high_score(self):
        ...
        if len(scores) < 5 or self.score > (scores[-1].get('score', 0) if scores else -1):
            self.root.update() 
            name = simpledialog.askstring("Nowy Rekord!", "Podaj swoje imię:", parent=self.root)
            if not name: name = "Gracz"
    ...
    def show_menu(self):
        ...
        self.canvas.create_text(150, 580, text=footer_text, fill="#AAAAAA", font=('Consolas', 8), justify="center")

    def show_top5(self):
        self.canvas.delete("all")
        scores = self.load_scores()
        self.canvas.create_text(150, 80, text="TOP 5 GRACZY", fill="#FFD700", font=('Consolas', 18, 'bold'))
        for i, s in enumerate(scores):
            ...
            self.canvas.create_text(250, 180 + (i * 40), anchor="e", text=f"{pts} PKT", fill="#00FFFF", font=('Consolas', 12))
        self.canvas.create_text(150, 550, text="ENTER = POWRÓT", fill="#BBBBBB", font=('Consolas', 10))

    def show_help(self):
        self.canvas.delete("all")
        self.canvas.create_text(150, 50, text="POMOC I OPIS", fill="#00FFFF", font=('Consolas', 18, 'bold'))
        
        # Opis gry
        desc = ("Tetris Mini to klasyczna gra logiczna.\n"
                "Układaj spadające klocki tak, aby\n"
                "tworzyły pełne linie poziome.\n"
                "Zapełniona linia znika, dając punkty.\n"
                "Gra kończy się, gdy klocki sięgną góry.")
        self.canvas.create_text(150, 130, text=desc, fill="#DDDDDD", font=('Consolas', 9), justify="center")

        # Sterowanie
        help_text = "STRZAŁKI LEWO/PRAWO: Ruch\nGÓRA: Obrót klocka\nDÓŁ: Szybkie opadanie"
        self.canvas.create_text(150, 250, text=help_text, fill="white", font=('Consolas', 10, 'bold'), justify="center")
        
        # Stopka
        author_info = ("author: Sebastian Januchowski\n"
                       "email: polsoft.its@fastservice.com\n"
                       "github: https://github.com/seb07uk\n\n"
                       "2026© polsoft.ITS London")
        self.canvas.create_text(150, 480, text=author_info, fill="#AAAAAA", font=('Consolas', 8), justify="center")
        self.canvas.create_text(150, 580, text="ENTER = POWRÓT", fill="#BBBBBB", font=('Consolas', 9))
    ...
    def handle_input(self, event):
        ...
        if self.state == "MENU":
            if key == "Up": self.menu_index = (self.menu_index - 1) % len(self.menu_options); self.show_menu()
                ...
                if choice == "NOWA GRA": self.start_game()
                elif choice == "TOP 5": self.state = "TOP5"; self.show_top5()
                elif choice == "POMOC": self.state = "HELP"; self.show_help()
                elif choice == "WYJSCIE": self.root.destroy()
